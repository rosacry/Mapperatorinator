$(document).ready(function () {
  ...
  const Utils = {
    ...
    resetFormToDefaults() {
      ...
      $('input[name="descriptors"], input[name="in_context_options"]')
        .removeClass("positive-check negative-check")
        .prop("checked", false);

      // Clear paths and optional fields
      $(
        "#audio_path, #output_path, #beatmap_path, #mapper_id, #seed, #start_time, #end_time, #hold_note_ratio, #scroll_speed_ratio"
      ).val("");
  ...
  const UIManager = {
    updateConditionalFields() {
      ...
      $(".conditional-field[data-show-for-gamemode]").each(function () {
        const $field = $(this);
        const supportedModes = $field
          .data("show-for-gamemode")
          .toString()
          .split(",");
      ...
      ["#in-context-options-box", "#add-to-beatmap-option"].forEach(
        (selector) => {
        ...
        try {
          ...
          if (browseType === "folder") {
            ...
            if (targetId === "beatmap_path") {
              fileTypes = ["Beatmap Files (*.osu)", "All files (*.*)"];
          ...
          if (path) {
            if (
              targetId === "beatmap_path" &&
              !path.toLowerCase().endsWith(".osu")
            ) {
              Utils.showFlashMessage(
                "Please select a valid .osu file.",
                "error"
              );
          ...
          alert(
            `Could not browse for ${browseType}. Ensure the backend API is running.`
          );
        }
      });
    },
  ...
  const PathManager = {
    ...
    validateAndAutofillPaths(showFlashMessages = false) {
      // isFileDialog replaced by showFlashMessages
        ...
        $.ajax({
          ...
          error: (xhr, status, error) => {
            ...
            if (showFlashMessages) {
              Utils.showFlashMessage(
                "Error validating paths. Check console for details.",
                "error"
              );
            }
            resolve(false);
    ...
    showInlineErrorForMessage(error) {
      ...
      if (
        error.includes("Audio file not found") &&
        (audioPathVal || beatmapPathVal)
      ) {
        ...
        error.includes("Beatmap file must have .osu extension") &&
        beatmapPathVal
      ) {
    ...
    showInlineError(inputSelector, message) {
      ...
      if ($input.siblings(".path-validation-error").length > 0) {
        ...
        const $errorDiv = $(
          `<div class="path-validation-error" style="color: #ff4444; font-size: 12px; margin-top: 2px;">${message}</div>`
        );
        ...
        function (e) {
          ...
          if (!$checkbox.prop("disabled")) {
            if ($checkbox.hasClass("positive-check")) {
              $checkbox
                .removeClass("positive-check")
                .addClass("negative-check");
  ...
  const ConfigManager = {
        ...
        .each(function () {
          ...
          if (name && type !== "file") {
            config.settings[name] =
              type === "checkbox" ? $field.prop("checked") : $field.val();
          }
        });
    ...
    async exportToFile(config) {
      try {
        const filename = `mapperatorinator-config-${new Date()
          .toISOString()
          .slice(0, 19)
          .replace(/:/g, "-")}.json`;
        ...
        $.ajax({
          ...
          success: (response) => {
            if (response.success) {
              this.showConfigStatus(
                `Configuration exported successfully to: ${response.file_path}`,
                "success"
              );
            } else {
              this.showConfigStatus(
                `Error saving config: ${response.error}`,
                "error"
              );
            }
          },
          error: () => {
            this.showConfigStatus(
              "Failed to save config to server. Using browser download instead.",
              "error"
            );
            this.fallbackDownload(config);
          },
    ...
    fallbackDownload(config) {
      ...
      a.href = url;
      a.download = `mapperatorinator-config-${new Date()
        .toISOString()
        .slice(0, 19)
        .replace(/:/g, "-")}.json`;
      ...
      this.showConfigStatus(
        "Configuration exported successfully (browser download)",
        "success"
      );
    },
    ...
    importConfiguration(content) {
      try {
        ...
        config.inContextOptions?.forEach((value) => {
          $(`input[name="in_context_options"][value="${value}"]`).prop(
            "checked",
            true
          );
        });
  ...
  const InferenceManager = {
    ...
    async validateForm() {
      ...
      if (!audioPath && !beatmapPath) {
        ...
        Utils.showFlashMessage(
          "Either 'Beatmap Path' or 'Audio Path' are required for running inference",
          "error"
        );
        return false;
      }
      ...
      if (!outputPath && !beatmapPath) {
        Utils.smoothScroll(0);
        Utils.showFlashMessage(
          "Either 'Output Path' or 'Beatmap Path' are required for running inference",
          "error"
        );
        return false;
      }
      ...
      if (beatmapPath && !beatmapPath.toLowerCase().endsWith(".osu")) {
        ...
        Utils.showFlashMessage(
          "Beatmap file must have .osu extension",
          "error"
        );
    ...
    resetProgress() {
      ...
      $("#init_message")
        .text("Initializing process... This may take a moment.")
        .show();
    ...
    buildFormData() {
      ...
      negativeDescriptors.forEach((val) =>
      ...
      if (
        $("#model").val() === "v30" &&
        !$("#option-item-hitsounded").is(":visible")
      ) {
    ...
    startInference() {
      $.ajax({
        ...
        error: (jqXHR, textStatus, errorThrown) => {
          ...
          let errorMsg =
            "Failed to start inference process. Check backend console.";
    ...
    handleSSEMessage(e) {
      ...
      const isErrorMessage = errorIndicators.some((indicator) =>
        messageData.includes(indicator)
      );
    ...
    handleSSEError(err) {
      ...
      if (!AppState.isCancelled && !AppState.inferenceErrorOccurred) {
        AppState.inferenceErrorOccurred = true;
        AppState.accumulatedErrorMessages.push(
          "Error: Connection to process stream lost."
        );
        $("#progressTitle")
    ...
    handleSSEEnd(e) {
      ...
      if (AppState.isCancelled) {
        $(
          "#progressTitle, #progressBarContainer, #beatmapLink, #errorLogLink"
        ).hide();
        $("#progress_output").hide();
      } else if (AppState.inferenceErrorOccurred) {
    ...
    handleInferenceError() {
      ...
      let specificError =
        "An error occurred during processing. Check console/logs.";

      if (fullErrorText.includes("FileNotFoundError:")) {
        const fileNotFoundMatch = fullErrorText.match(
          /FileNotFoundError:.*? file (.*?) not found/
        );
        specificError = fileNotFoundMatch?.[1]
          ? `Error: File not found - ${fileNotFoundMatch[1].replace(
              /\\\\/g,
              "\\\\"
            )}`
        ...
        specificError =
          "There was an error while creating the beatmap. Check console/logs for details.";
      } else if (fullErrorText.includes("Error executing job")) {
        specificError =
          "There was an error starting or executing the generation task.";
    ...
    cancelInference() {
      ...
      $.ajax({
        ...
        success: (response) => {
          ...
          Utils.showFlashMessage(
            response.message || "Inference cancelled successfully.",
            "cancel-success"
          );
        },
        error: (jqXHR) => {
          const errorMsg =
            ...
            "Failed to send cancel request. Unknown error.";
          Utils.showFlashMessage(errorMsg, "error");
