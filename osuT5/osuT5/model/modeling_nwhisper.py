...
_CONFIG_FOR_DOC = "NWhisperConfig"
...
def l2norm(
    ...
    if norm_eps == 0.:
        out = nn.functional.normalize(t, dim=dim, p=2)
    else:
...
class NWhisperPositionalEmbedding(nn.Embedding):
...
class NWhisperAttention(nn.Module):
    ...
    def __init__(
            ...
            config: Optional[NWhisperConfig] = None,
            s_qk_init=1.,
            s_qk_scale=None,
    ):
        super().__init__()
        ...
        NormLinear_ = partial(NormLinear, parametrize=not config.manual_norm_weights, norm_eps=config.norm_eps,
                              groups=config.num_hyperspheres)
        self.l2norm = partial(l2norm, norm_eps=config.norm_eps, groups=config.num_hyperspheres)
...
class NWhisperFlashAttention2(NWhisperAttention):
    """
    NWhisper flash attention module. This module inherits from `NWhisperAttention` as the weights of the module stays
    ...
    def forward(
        ...
        # NWhisperFlashAttention2 attention does not support output_attentions
        if output_attentions:
            raise ValueError("NWhisperFlashAttention2 attention does not support output_attentions")
...
class NWhisperSdpaAttention(NWhisperAttention):
    def forward(
            self,
            hidden_states: torch.Tensor,
            key_value_states: Optional[torch.Tensor] = None,
        ...
        if output_attentions or layer_head_mask is not None:
            ...
            logger.warning_once(
                "NWhisperModel is using NWhisperSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True` or `layer_head_mask` not None. Falling back to the manual attention"
...
WHISPER_ATTENTION_CLASSES = {
    "eager": NWhisperAttention,
    "flash_attention_2": NWhisperFlashAttention2,
    "sdpa": NWhisperSdpaAttention,
}
...
class FeedForward(Module):
    def __init__(
        ...
        self.fc1 = NormLinear_(dim, dim_inner)
        self.fc2 = NormLinear_(dim_inner, dim, norm_dim_in=False)

    def forward(self, x):
        hidden_states = self.activation_fn(self.fc1(x))
...
# Copied from transformers.models.mbart.modeling_mbart.MBartEncoderLayer with MBart->NWhisper, MBART->WHISPER
class NWhisperEncoderLayer(nn.Module):
    def __init__(
            self,
            config: NWhisperConfig,
        ...
        self_attn = WHISPER_ATTENTION_CLASSES[config._attn_implementation](
        ...
        self.attn_with_residual = Residual(
            self_attn,
            self.embed_dim,
            default(alpha_attn_init, config.alpha_init),
            default(alpha_attn_scale, self.embed_dim ** -0.5)
        )

        self.ff_with_residual = Residual(
            ff,
            self.embed_dim,
            default(alpha_ff_init, config.alpha_init),
            default(alpha_ff_scale, self.embed_dim ** -0.5)
        )
    ...
    def forward(
        ...
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        hidden_states = self.ff_with_residual(hidden_states)
...
class NWhisperDecoderLayer(nn.Module):
    def __init__(
            self,
            config: NWhisperConfig,
            layer_idx: int = None,
        ...
        self.self_attn_with_residual = Residual(
            self_attn,
            self.embed_dim,
            default(alpha_attn_init, config.alpha_init),
            default(alpha_attn_scale, self.embed_dim ** -0.5)
        ...
        self.encoder_attn_with_residual = Residual(
            encoder_attn,
            self.embed_dim,
            default(alpha_cross_attn_init, config.alpha_init),
            default(alpha_cross_attn_scale, self.embed_dim ** -0.5)
        )
...
class NWhisperPreTrainedModel(PreTrainedModel):
    config_class = NWhisperConfig
    base_model_prefix = "model"
    main_input_name = "input_features"
    supports_gradient_checkpointing = True
    _no_split_modules = ["NWhisperEncoderLayer", "NWhisperDecoderLayer"]
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, NWhisperEncoder):
            with torch.no_grad():
...
class NWhisperEncoder(NWhisperPreTrainedModel):
    """
    Transformer encoder consisting of *config.encoder_layers* self attention layers. Each layer is a
    [`NWhisperEncoderLayer`].
    ...
    def __init__(self, config: NWhisperConfig):
        ...
        scale_hparams = (
            ...
            config.encoder_alpha_attn_scale,
            config.encoder_alpha_ff_init,
            config.encoder_alpha_ff_scale,
            config.encoder_s_qk_init,
            config.encoder_s_qk_scale,
        )

        scale_hparams = tuple(cast_tuple(hparam, config.encoder_layers) for hparam in scale_hparams)

        self.layers = nn.ModuleList([NWhisperEncoderLayer(
            config,
            alpha_attn_init=alpha_attn_init_,
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask (`torch.Tensor`)`, *optional*):
                NWhisper does not support masking of the `input_features`, this argument is preserved for compatibility,
        ...
        if input_features.shape[-1] != expected_seq_length:
            raise ValueError(
                f"NWhisper expects the mel input features to be of length {expected_seq_length}, but found {input_features.shape[-1]}. Make sure to pad the input mel features to {expected_seq_length}."
            )
...
class NWhisperDecoder(NWhisperPreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a [`NWhisperDecoderLayer`]
    ...
    def __init__(self, config: NWhisperConfig):
        ...
        scale_hparams = (
            ...
            config.decoder_alpha_attn_init,
            config.decoder_alpha_attn_scale,
            config.decoder_alpha_cross_attn_init,
            config.decoder_alpha_cross_attn_scale,
            config.decoder_alpha_ff_init,
            config.decoder_alpha_ff_scale,
            config.decoder_s_qk_init,
            config.decoder_s_qk_scale,
            config.decoder_cross_s_qk_init,
            config.decoder_cross_s_qk_scale,
        )

        scale_hparams = tuple(cast_tuple(hparam, config.decoder_layers) for hparam in scale_hparams)

        self.layers = nn.ModuleList([NWhisperDecoderLayer(
            config,
            layer_idx,
            alpha_attn_init=alpha_attn_init_,
            alpha_attn_scale=alpha_attn_scale_,
            alpha_cross_attn_init=alpha_cross_attn_init_,
            ...
            alpha_attn_init_,
            alpha_attn_scale_,
            alpha_cross_attn_init_,
            alpha_cross_attn_scale_,
            alpha_ff_init_,
            alpha_ff_scale_,
            s_qk_init_,
            s_qk_scale_,
...
@add_start_docstrings(
    "The bare NWhisper Model outputting raw hidden-states without any specific head on top.",
    WHISPER_START_DOCSTRING,
)
class NWhisperModel(NWhisperPreTrainedModel):
    def __init__(self, config: NWhisperConfig):
        super().__init__(config)

        self.encoder = NWhisperEncoder(config)
        self.decoder = NWhisperDecoder(config)
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def freeze_encoder(self):
        """
        Calling this function will disable the gradient computation for the NWhisper encoder so that its parameters will
...
@add_start_docstrings(
    "The NWhisper Model with a language modeling head. Can be used for automatic speech recognition.",
    WHISPER_START_DOCSTRING,
)
class NWhisperForConditionalGeneration(WhisperGenerationMixin, NWhisperPreTrainedModel):
    base_model_prefix = "model"
    ...
    def prepare_inputs_for_generation(
        ...
        # Overwritten -- encoder-decoder nwhisper has custom logic, but it's close to the general function. Next time
...
class NWhisperDecoderWrapper(NWhisperPreTrainedModel):
    """
...
@add_start_docstrings(
    """
    NWhisper decoder with a language modeling head on top (linear layer with weights tied to the input embeddings).
    """,
    WHISPER_START_DOCSTRING,
)
class NWhisperForCausalLM(NWhisperPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["proj_out.weight"]
    main_input_name = "input_ids"
    ...
    def __init__(self, config):
        super().__init__(config)
        config.is_encoder_decoder = False
        self.model = NWhisperDecoderWrapper(config)
...
@add_start_docstrings(
    """
    NWhisper Encoder Model with a sequence classification head on top (a linear layer over the pooled output) for tasks
    like SUPERB Keyword Spotting.
    """,
    WHISPER_ENCODER_INPUTS_DOCSTRING,
)
class NWhisperForAudioClassification(NWhisperPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
