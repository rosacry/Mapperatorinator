...
def forward(model: BertForSequenceClassification, batch):
    outputs = model(**batch)
    loss = outputs.loss
...
def forward_eval(model: BertForSequenceClassification, batch):
    outputs = model(**batch)
    return outputs
